...
class Squad {
	...
	unitReady() {
		if (this.ready === 0) {
			this.bar = this.simulator.multiprogress.newBar(`[RECHARGE] Recharging Squad ${this.squadIndex} from army '${this.army.name}' :bar :current/:total`, { total: this.getActiveUnits().length, width: 20 });
		}
		this.bar.tick();
		this.ready++;
	...
	receiveDamage(damage) {
		...
		console.log(`[DAMAGE] Squad ${this.squadIndex} from army '${this.army.name}' received ${damage} damage`)
